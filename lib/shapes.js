...
export class Shape {
  constructor(sides, config) {
    ...
    for (let property of ["c", "ctx", "sides", "angle", "fpsArr", "fps", "rotations", "loop", "radius", "inradius", "height", "points"]) {
      Object.defineProperty(this, property, {
        get: function () {
          return secret[key][property];
        }
    ...
    let shape = (function shape(x, y, z, add) {
      ...
      for (let side = 0; side <= this.sides; side++) {
        ...
        secret[key].points.push(point);
        this.ctx.lineTo(...point);
      }
      secret[key].points.pop();
    }).bind(this);

    let clearApi = (function (save, stop, add) {
      if (this.show) {
    ...
    let drawApi = (function (x = 0, y = 0, z = 0) {
      this.clear();
      shape(x, y, z, 0);
    ...
    this.spin = (axis, input1, input2, start) => {
      ...
      if (this.fpsOut) {
        secret[key].fpsRecord = startTime;
        record = true;
      }
      ...
      secret[key].loop = setInterval(() => {
        ...
        if (record) {
          this.fpsOut(1000 / (secret[key].fpsRecord - now));
          secret[key].fpsRecord = now;
        }
