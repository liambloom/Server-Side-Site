...
let spinInput = (input1, input2) => { // processes 2 inputs (speed, time, or distance) and outputs 3. Also converts units
  ...
  if (ms === undefined) ms = degrees / dpms;
  else if (dpms === undefined) dpms = degrees / ms;
  else degrees = ms * dpms;
  if (dpms === undefined || ms === undefined || degrees === undefined) throw "Not enouph information to spin shape";
