/*
  ...
  Performance Issues:
    _shapeVars.axes is very ineficient
...
function drawFrame () {
  Shape.refreshAll();
  fpsTracker.push(performance.now());
  slowFpsArr.push(Shape.fps);
...
export default class Shape {
  ...
  static get fps () {
    if (Shape.spinning.length) {
      const now = performance.now();
      fpsTracker = fpsTracker.filter(time => now - time <= 1000);
