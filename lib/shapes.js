...
let spin = () => {
  loop = setInterval(() => {
    for (let i in spinning) {
      (function() {
        let now = performance.now();
        if (now >= spinning[i].end) { // if time is up
          if (spinning[i].disapear) this.clear();
          else this.stop();
        }
        else {
          spinning[i].clearApi.call(this, false, false, 1);
          let angle = (now - spinning[i].startTime) * spinning[i].dpms + spinning[i].startTime; // calculate angle based on time
          if (angle % 360 < 180 || spinning[i].axes.length === 0) spinning[i].axes.push(angle);
          else spinning[i].axes.push(angle + 180); // the up part of a cosine wave messes up clear function for x and y spinning
          spinning[i].drawApi.apply(this, spinning[i].axes);
          spinning[i].axes.pop();
          if (this.onframe) this.onframe(angle % 360);
        }
      }).call(spinning[i].shape);
    }
  }, 1000 / spinning[0].fps);
};
...
export class Shape {
  constructor(sides, config) {
    ...
    for (let property of ["c", "ctx", "sides", "angle", "fpsArr", "fps", "radius", "inradius", "height", "points", "onframe"]) {
      Object.defineProperty(this, property, {
    ...
    Object.defineProperties(this, {
      ...
      spinning: {
        get: function () { // check if this shape is spinning, and return a boolean value
          return !!spinning.find(e => e.shape === this);
        }
      },
      y: {
        get: function () {
        ...
        set: function (value) {
          if (this.show && !this.spinning) clearApi(true);
          let y;
    ...
    secret[key] = { // easily set a few values
      ...
      onframe: (typeof config.onframe === "function") ? config.onframe.bind(this) : false
    };
    ...
    this.toFront = () => {
      if (this.spinning) {
        spinning.push(spinning.splice(spinning.indexOf(spinning.find(e => e.shape === this)), 1)[0]);
      }
    };
    this.refresh = () => {
      if (this.show) {
    ...
    this.stop = () => {
      let thisIndex = spinning.indexOf(spinning.find(e => e.shape === this));
      if (thisIndex !== -1) {
        spinning.splice(thisIndex, 1);
