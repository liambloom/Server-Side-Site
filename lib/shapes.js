...
export class Shape {
  constructor(sides, config) {
    ...
    for (let property of ["c", "ctx", "sides", "angle", "fpsArr", "fps", "loop", "points"]) {
      Object.defineProperty(this, property, {
  ...
  set y (value) {
    if (this.show && this.loop === -1) this._clearApi(true);
  ...
  set rotations (value) {
    if (this.show && Array.isArray(value)) {
      ...
      if (this.show && this.loop === -1) this.clear();
  ...
  spin (axis, input1, input2, start) { // spin the shape
    ...
    secret[this._key].loop = setInterval(() => {
  ...
  stop () {
    ...
    secret[this._key].loop = -1;
    secret[this._key].fpsRecord = undefined;
  }
}
  ...
  _makeGrid () {
    for (let x = this.minX; x <= this.maxX; x += this.radius + (this.side / 2)) {
      ...
      for (let y = this.minY + offset; y <= this.maxY; y += this.shapeHeight) {
        this.config.x = x;
        this.config.y = y;
        let hex = new Shape(6, this.config);
